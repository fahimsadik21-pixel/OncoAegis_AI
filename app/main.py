...
from app.api.flowcap_aml_api import router as flowcap_aml_router
from app.api.tn3k_thyroid_api import router as tn3k_thyroid_router
...
from app.imaging.hematology.cnmc_visualization import (
    save_cnmc_visualization,
)

from app.imaging.hematology.cnmc_output_export import (
    export_cnmc_result,
)
...
app = FastAPI(
    title="Onco Aegis AI",
    description="Cancer information and multimodal analysis assistant",
    version=APP_VERSION,
)

app.include_router(flowcap_aml_router)
app.include_router(tn3k_thyroid_router)
...
_cnmc_service: CNMCAnalysisService | None = None


async def _read_specialist_uploads(
    files: list[UploadFile],
) -> list[UploadedSpecialistFile]:
    """Read uploads with bounded memory before specialist normalization."""

    if len(files) > MAX_SPECIALIST_FILES:
        raise HTTPException(
            status_code=413,
            detail=f"Too many uploaded files; maximum is {MAX_SPECIALIST_FILES}",
        )

    total_bytes = 0
    uploaded: list[UploadedSpecialistFile] = []
    for index, file in enumerate(files, start=1):
        declared_size = getattr(file, "size", None)
        if (
            declared_size is not None
            and int(declared_size) > MAX_SPECIALIST_FILE_BYTES
        ):
            raise HTTPException(
                status_code=413,
                detail=(
        ...
        content = await file.read(MAX_SPECIALIST_FILE_BYTES + 1)
        if not content:
            raise HTTPException(
                status_code=400,
        ...
        uploaded.append(
            UploadedSpecialistFile(
                name=file.filename or f"upload_{index}",
                content=content,
            )
        )
    return uploaded
...
def _storage_actor(actor_id: object | None) -> str:
    return normalize_actor_id(actor_id)


def _require_storage_admin(admin_token: str | None) -> None:
    configured_token = os.getenv("ONCOAEGIS_STORAGE_ADMIN_TOKEN")
    if not configured_token:
        raise HTTPException(
            status_code=503,
            detail=(
                "Destructive storage operations are disabled until "
                "ONCOAEGIS_STORAGE_ADMIN_TOKEN is configured"
            ),
        )
    if not admin_token or not hmac.compare_digest(admin_token, configured_token):
        raise HTTPException(status_code=403, detail="Storage administrator token required")


def get_cnmc_service() -> CNMCAnalysisService:
...
def system_status():
    ...
    return {
        ...
        "components": {
            ...
            "clinical_document_reasoning": "structured_extraction_with_ocr_boundary",
            "multimodal_fusion": "reasoning_ready",
            "case_storage": "owner_scoped_with_retention_and_audit",
            "authentication": "ready",
            "cancer_information": "safe_fallback_ready",
...
@app.post(
    "/analyze/specialist",
    response_model=StandardAnalysisResult,
)
async def analyze_specialist(
    ...
    """Run any available registered specialist through one safe contract."""
    ...
    try:
        ...
        raise HTTPException(
            status_code=422,
            detail=(
                "Specialist analysis could not be completed safely: "
                f"{type(exc).__name__}"
            ),
        ) from exc
    ...
    try:
        ...
        raise HTTPException(
            status_code=503,
            detail="Analysis completed but result storage was unavailable",
        ) from exc
    return result
...
def storage_policy(
    ...
    """Expose the active storage and retention contract without DB secrets."""
...
def list_cases(
    ...
    """Return privacy-safe case history summaries."""
...
def get_case_result(
    ...
    """Return one stored StandardAnalysisResult and its integrity metadata."""
    ...
    except (CaseNotFoundError, CaseDeletedError) as exc:
        raise HTTPException(status_code=404, detail="Stored result not found") from exc
    except InvalidCaseIDError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    except StorageError as exc:
        raise HTTPException(status_code=503, detail="Stored result is unavailable") from exc
...
def list_case_results(
    ...
    """Return stored result summaries for one case."""
    ...
    try:
        ...
        raise HTTPException(status_code=404, detail="Case results not found") from exc
    except InvalidCaseIDError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    except StorageError as exc:
...
def list_case_audit(
    ...
    """Return the append-only audit events associated with a case."""
    ...
    try:
        ...
        raise HTTPException(status_code=404, detail="Case audit not found") from exc
    except InvalidCaseIDError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    except StorageError as exc:
...
def get_case_history(
    ...
    """Return one case and its stored result history."""

    try:
        return get_case_store().get_case(
            case_id,
        ...
        raise HTTPException(status_code=410, detail="Case has been deleted") from exc
    except InvalidCaseIDError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    except StorageError as exc:
        raise HTTPException(status_code=503, detail="Case history is unavailable") from exc
...
def delete_case(
    ...
    admin_token: str | None = Header(
        default=None,
        alias="X-Storage-Admin-Token",
    ),
    current_user = Depends(require_permission(Permission.MANAGE_USERS)),
):
    """Purge case result payloads while retaining an auditable tombstone."""
    ...
    try:
        ...
        raise HTTPException(status_code=410, detail="Case has already been deleted") from exc
    except InvalidCaseIDError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    except StorageError as exc:
...
def purge_expired_cases(
    ...
    """Run the explicit retention job; it never runs silently at import time."""

    _require_storage_admin(admin_token)
    try:
        return get_case_store().purge_expired_cases(
            actor_id=current_user.user_id
        )
    except StorageError as exc:
        raise HTTPException(status_code=503, detail="Retention job is unavailable") from exc


@app.get("/admin/storage/audit/verify")
def verify_storage_audit(
    admin_token: str | None = Header(
        default=None,
        alias="X-Storage-Admin-Token",
    ),
):
    """Verify the global audit hash chain without exposing stored payloads."""

    _require_storage_admin(admin_token)
    try:
        return get_case_store().verify_audit_chain()
    except StorageError as exc:
        raise HTTPException(status_code=503, detail="Audit verification is unavailable") from exc
...
@app.post("/analyze/hematology-cell")
async def analyze_hematology_cell(
    file: UploadFile = File(...),
):
    ...
    except CNMCAnalysisError as exc:

        raise HTTPException(
            status_code=422,
            detail=f"Hematology analysis failed: {exc}",
        ) from exc
    ...
    return {
        ...
        "interpretation": {
            ...
            "all_probability":
                result.all_probability,

            "hem_probability":
                result.hem_probability,

            "meaning": (
                "This research model classifies a single "
                "microscopy cell image as ALL-like or HEM-like. "
                "It does not diagnose leukemia at patient level."
            ),
        },
